...
public class AnnotationBeanNameGenerator implements BeanNameGenerator {
	...
	/**
	 * 默认{@code注释beannamegenerator}实例的一个方便常量，
	 * *用于组件扫描目的。
	...
	public String generateBeanName(BeanDefinition definition, BeanDefinitionRegistry registry) {
		...
		// Fallback: generate a unique default bean name. 回退:生成唯一的默认bean名称。
	 ...
	 * 从给定bean定义派生默认bean名称。
	 * * <p>默认实现只是构建一个非资本化版本
	 * *的短类名称:例如。“mypackage。MyJdbcDao”- >“MyJdbcDao”。
	 * 注意，内部类将因此拥有表单的名称
	 * *“outerClassName。中的句点
	 * *名称可能是一个问题，如果你是自动名称。
	 * * @param定义为其构建bean名称的bean定义
	 * * @返回默认的bean名称(never {@code null})
	 * Derive a default bean name from the given bean definition.
