...
/**
 * 配置接口将被大多数{@link环境}类型实现。
 * *提供设置活动和默认配置文件和操作底层的设施
 * *财产来源。允许客户端设置和验证所需的属性，自定义
 * *转换服务和更多通过{@link ConfigurablePropertyResolver}
 * *超接口。
 * *
 * * <h2>操作属性源</h2>
 * * >属性源可能被删除、重新排序或替换;和额外的
 * *可以使用{@link MutablePropertySources}添加属性源
 * 实例从{@link #getPropertySources()}返回。下面的例子
 * *是反对{@link StandardEnvironment}实现的
 * * {@code ConfigurableEnvironment}，但一般适用于任何实现，
 * *尽管特定的默认属性来源可能不同。
 * *
 * * <h4>示例:添加一个最高搜索优先级</h4>的新属性源
 * * < pre类=“代码”>
 * * ConfigurableEnvironment environment = new StandardEnvironment();
 * * MutablePropertySources = environment.getPropertySources();
 * * Map&lt;字符串,String&gt;myMap = new HashMap&lt;&gt;();
 * *关联。把(“xyz”、“括号”);
 * * propertySources。addFirst(新MapPropertySource (“MY_MAP”,关联));
 * * < / pre >
 * *
 ...
 * * < / pre >
 * *
 * * <h4>示例:mock用于测试的系统环境</h4>
 * * < pre类=“代码”>
 * * MutablePropertySources = environment.getPropertySources();
 * * MockPropertySource mockEnvVars = new MockPropertySource()withProperty (“xyz”、“括号”);
 * * propertySources.replace (StandardEnvironment。SYSTEM_ENVIRONMENT_PROPERTY_SOURCE_NAME mockEnvVars);
 * * < / pre >
 * *
 * 当{@code ApplicationContext}正在使用{@link环境}时，它是
 * *重要的是要执行任何这样的{@code PropertySource}操作
 * * <em>之前</em>上下文的{@link
 * * org.springframework.context.support.AbstractApplicationContext #刷新刷新()()}
 * 方法被调用。这确保了所有属性源在
 * *容器引导过程，包括使用{@linkplain
 * * org.springframework.context.support。PropertySourcesPlaceholderConfigurer财产
 * *占位符configurers}。
