...
public interface MergedAnnotations extends Iterable<MergedAnnotation<Annotation>> {
	...
	/**
	 * 创建一个新的{@link MergedAnnotations}实例，其中包含所有
	 * *指定元素的注释和元注释，
	 * *根据{@link SearchStrategy}，相关的继承元素。
	 * * @param元素:源元素
	 * * @param搜索策略使用的搜索策略
	 * repeatableContainers可被使用的可重复容器
	 * *元素注释或元注释
	 * 一个注释过滤器，用于限制
	 * *注释被认为
	 * * @返回一个{@link MergedAnnotations}实例，该实例包含被合并的
	 * *元素注释
	 * Create a new {@link MergedAnnotations} instance containing all
	 * annotations and meta-annotations from the specified element and,
	 * depending on the {@link SearchStrategy}, related inherited elements.
	 * @param element the source element 元素
	 * @param searchStrategy the search strategy to use 使用的搜索策略
	 * @param repeatableContainers the repeatable containers that may be used by
	 * 	 * the element annotations or the meta-annotations 可重复使用的容器
	 * *元素注释或元注释
	 * @param annotationFilter an annotation filter used to restrict the
	 * annotations considered 用于限制的注释筛选器
	 * *注释被认为
	 * @return a {@link MergedAnnotations} instance containing the merged
	 * element annotations 实例，其中包含合并的
	 * *元素注释
	 */
	static MergedAnnotations from(AnnotatedElement element, SearchStrategy searchStrategy,
	...
	enum SearchStrategy {
		...
		/**
		 * 查找所有直接声明的注释以及任何注释
		 * * {@link Inherited @Inherited}超类注释。这个策略
		 * *只有在与{@link Class}类型一起使用时才真正有用
		 * * {@link Inherited @Inherited}注释被忽略
		 * * {@linkplain AnnotatedElement annotated elements}。这个策略
		 * *不搜索实现的接口。
