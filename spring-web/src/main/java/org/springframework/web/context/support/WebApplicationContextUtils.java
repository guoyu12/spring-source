...
public abstract class WebApplicationContextUtils {
	...
	/**
	 * 将基于{@code Servlet}的{@link StubPropertySource存根属性源}替换为
	 * *使用给定的{@code servletContext}和填充的实际实例
	 * * {@code servletConfig}对象。
	 * 这个方法是幂等的，因为它可以被称为任何数
	 * *的时间，但将执行替换存根属性源
	 * *对应的实际属性来源一次且仅一次。
	 * * @param源{@link MutablePropertySources}初始化(必须不
	 * * be {@code null})
	 * * @param servletContext当前{@link servletContext}(忽略如果{@code null})
	 * *或者如果{@link StandardServletEnvironment#SERVLET_CONTEXT_PROPERTY_SOURCE_NAME
	 * * servlet上下文属性源}已经初始化)
	 * * @param servletConfig当前的{@link servletConfig}(忽略如果{@code null})
	 * *或者如果{@link StandardServletEnvironment#SERVLET_CONFIG_PROPERTY_SOURCE_NAME
	 * * servlet配置属性源}已经初始化)
